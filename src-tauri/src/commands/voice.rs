...
pub fn response_instructions(depth: &str) -> String {
    let limit = match depth { "BRIEF" => 110, "DEEP_DIVE" => 90, _ => 55 };
    format!(r#"
...
pub fn parse_answer(raw: &str, depth: &str) -> Result<VoiceAnswer, String> {
    ...
    let max_words = match depth { "BRIEF" => 110, "DEEP_DIVE" => 90, _ => 55 };
...
pub fn session_config() -> Value {
    json!({"expires_after":{"anchor":"created_at","seconds":60},"session":{
        "type":"realtime","model":MODEL,"instructions":SPEECH_INSTRUCTIONS,
        "output_modalities":["audio"],"max_output_tokens":800,
        "audio":{"input":{"transcription":{"model":"gpt-4o-mini-transcribe","language":"en"},
            "noise_reduction":{"type":"near_field"},
            "turn_detection":{"type":"server_vad","threshold":0.5,"prefix_padding_ms":300,"silence_duration_ms":650,"create_response":false,"interrupt_response":true}},
...
#[tauri::command]
pub async fn create_voice_session() -> Result<ClientSecret, String> {
    ...
    let response = client.post("https://api.openai.com/v1/realtime/client_secrets")
        .bearer_auth(key.trim()).json(&session_config()).send().await.map_err(|_| "Could not connect to OpenAI voice. Check your connection and try again.")?;
...
mod tests {
    use super::*;
    #[test] fn session_does_not_auto_answer() { let v=session_config(); assert_eq!(v["session"]["audio"]["input"]["turn_detection"]["create_response"],false); assert!(v["session"].get("tools").is_none()); }
