...
const makeRun=():IntelligenceRun=>({id:'synthetic-run',graph:'communication-intelligence/v2',status:scenario==='analysis-failed'?'failed':'completed',startedAt:new Date(now).toISOString(),finishedAt:new Date(now+18).toISOString(),durationMs:18,days:7,stale:scenario==='stale',error:scenario==='analysis-failed'?'Synthetic source unavailable':null,skills:[{id:'communication-assess',version:1,implementation:'Synthetic fixture only'}],definition:[{id:'snapshot',kind:'cache_read',dependsOn:[],maxIterations:1},{id:'select',kind:'candidate_selection',dependsOn:['snapshot'],maxIterations:1},{id:'assess',kind:'communication-assess@1',dependsOn:['select'],maxIterations:1},{id:'project',kind:'project-relevance@2',dependsOn:['select'],maxIterations:1},{id:'synthesize',kind:'validated_join_and_policy',dependsOn:['assess','project'],maxIterations:1}],items:scenario==='empty'?[]:rows.slice(0,3).map((r,i)=>{const evidenceRefs=[{messageId:r.id,threadId:r.threadId,fingerprint:r.fingerprint,timestamp:r.timestamp}];return {threadId:r.threadId,subject:r.subject,sender:r.sender,triage:{attention:'yes',summary:'Possible response needed.',reasonCodes:['possible_response_needed'],evidenceRefs},summary:{whatHappened:'Latest source: Can you review the plan?',whatChanged:'One cached message inspected; no earlier comparison available.',whatMatters:i===1?'Possible deadline language needs verification.':'A received message may need your response.',likelyNextMove:'Review the source.',evidenceRefs},project:{state:'suggested',reason:'Generated project candidate; not operator-confirmed.',ambiguity:false,suggestedProjects:[{name:'Olympus',source:'01 - Projects/Olympus.md',fingerprint:'fixture-project'}],method:'bounded_name_alias_match'},recommendation:{disposition:i===1?'verify':'review',guidance:i===1?'Verify the possible deadline and whether it applies to you.':'Review the latest thread before deciding whether to reply.',priority:'attention_candidate',evidenceRefs}}})});
...
async function run() { const checks: string[] = []; const check = (v: unknown, s: string) => { if (!v)
    ...
    check(document.querySelector('.comms-run-details')?.textContent?.includes('communication-assess@1'),'Registry and graph use the consolidated assessment skill');
